-- ============================================
-- ACCOUNTS RECEIVABLE - COMPLETE SETUP
-- ============================================
-- Execute these commands in order to set up AR module
...
-- Add comments
COMMENT ON TABLE customers IS 'Customers for Accounts Receivable (AR) - revenue invoices';
COMMENT ON COLUMN customers.code IS 'Customer code (Primary Key) - format: {country}-CU##### (e.g., ES-CU00001)';
COMMENT ON COLUMN customers.name IS 'Customer name';
COMMENT ON COLUMN customers.tax_id IS 'Tax ID / VAT number';
COMMENT ON COLUMN customers.payment_terms IS 'Payment terms (net_15, net_30, net_45, net_60, immediate, etc.)';
COMMENT ON COLUMN customers.credit_limit IS 'Credit limit in customer currency';


-- ============================================
-- 2. ADD CUSTOMER_CODE TO INVOICES TABLE
-- ============================================
...
COMMENT ON COLUMN invoices.customer_code IS 'Customer code for revenue invoices (AR) - references customers table';


-- ============================================
-- 3. INSERT REVENUE FINANCIAL ACCOUNTS (Series 100)
-- ============================================
...
-- ============================================
-- 4. UPDATE INVOICE_TYPE VALUES
-- ============================================
-- Add comment to clarify invoice_type usage
COMMENT ON COLUMN invoices.invoice_type IS 'Invoice type: INCURRED (AP/expense), REVENUE (AR/income), ACCRUED_REVENUE, DEFERRED_REVENUE';


-- ============================================
-- 5. CREATE SAMPLE CUSTOMERS (Optional)
-- ============================================
-- Uncomment to create sample customers for testing

-- INSERT INTO customers (code, name, tax_id, email, country, currency, payment_terms, is_active)
-- VALUES
--   ('ES-CU00001', 'Cliente Teste Espanha', 'B12345678', 'cliente1@example.com', 'ES', 'EUR', 'net_30', true),
--   ('US-CU00001', 'Test Customer USA', '12-3456789', 'customer1@example.com', 'US', 'USD', 'net_15', true);
...
-- Check revenue financial accounts
-- SELECT code, name, type, level, parent_code 
-- FROM financial_accounts 
-- WHERE type = 'revenue' 
-- ORDER BY code;

-- Check invoices with customers
-- SELECT id, invoice_type, customer_code, provider_code, financial_account_code, invoice_amount
-- FROM invoices
