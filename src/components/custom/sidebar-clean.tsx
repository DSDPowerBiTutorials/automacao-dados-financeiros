  ...
  const renderFlyout = () => {
    ...
    const findItem = (items: NavItem[]): NavItem | null => {
      for (const item of items) {
        if (item.href === hoveredItem) return item;
        if (item.children) {
    ...
    let targetItem: NavItem | null = null;
    for (const group of NAV) {
      targetItem = findItem(group.items);
      if (targetItem) break;
    }

    if (!targetItem || !targetItem.children || targetItem.children.length === 0) return null;

    return (
      <div
        className="fixed bg-[#1e293b] border border-gray-700 rounded-2xl shadow-2xl z-60 p-2 min-w-[240px]"
      ...
      >
        ...
        <div className="space-y-1">
          {targetItem.children.map((child) => renderFlyoutItem(child))}
        </div>
      </div>
    );
  };
    ...
    if (item.children && item.children.length > 0) {
      return (
        <div key={item.href} className="relative group/submenu">
          <div className={cn(itemClass(false), "justify-between cursor-pointer")}>
        ...
        <div className="border-t border-gray-700">
          {(!collapsed || isHovering) && (
            <>
              ...
              <button 
                className="flex items-center gap-3 px-4 py-3 w-full hover:bg-gray-700/50 text-gray-300 hover:text-white transition-all text-sm rounded-xl mx-2 mb-2"
          ...
          {(collapsed && !isHovering) && (
            <button
              className="flex items-center justify-center py-4 w-full hover:bg-gray-700/50 text-gray-300 hover:text-white transition-all"
