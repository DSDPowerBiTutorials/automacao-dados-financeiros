...
export default function Home() {
  ...
  const [categoryOptions] = useState<string[]>(["Revenue", "Expense", "Other"]);
  ...
  const handleFileUpload = async (
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (source === "braintree-amex") {
            ...
            const settlementDate = row["Settlement date"]
              ? convertDateFormat(row["Settlement date"])
              : convertDateFormat(new Date().toLocaleDateString("pt-BR"));

            // Pegar Amount Paid To Bank como payout
            const amountPaidToBank = parseMonetaryValue(
              row["Amount Paid To Bank"] || "0",
            );
            ...
            newRows.push({
              ...
              submission_amount: parseMonetaryValue(
                row["Submission amount"] || "0",
              ),
              discount_amount: parseMonetaryValue(
                row["Discount amount"] || "0",
              ),
              fees_and_incentives: parseMonetaryValue(
                row["Fees and incentives"] || "0",
              ),
              ...
              settlement_amount: parseMonetaryValue(
            ...
            const payoutDate = payoutDateRaw
              ? convertDateFormat(payoutDateRaw)
              : convertDateFormat(new Date().toLocaleDateString("pt-BR"));
            ...
            const arrivalDate = arrivalDateRaw
              ? convertDateFormat(arrivalDateRaw)
              : convertDateFormat(new Date().toLocaleDateString("pt-BR"));
            ...
            const disbursementDate = row["disbursement_date"]
              ? convertDateFormat(row["disbursement_date"])
              : convertDateFormat(new Date().toLocaleDateString("pt-BR"));

            // Calcular Payout (soma das colunas especificadas)
            const settlementSales =
              parseFloat(row["settlement_currency_sales_EUR"]) || 0;
            const discount = parseFloat(row["discount_EUR"]) || 0;
            const multicurrencyFees =
              parseFloat(row["multicurrency_fees_EUR"]) || 0;
            const perTransactionFees =
              parseFloat(row["per_transaction_fees_EUR"]) || 0;
            const crossBorderFees =
              parseFloat(row["cross_border_fees_EUR"]) || 0;
            const payout =
              settlementSales +
              discount +
              multicurrencyFees +
            ...
            newRows.push({
              ...
              chargebacks_lost_won_EUR:
                parseFloat(row["chargebacks_lost_won_EUR"]) || 0,
              settlement_currency_refunds_EUR:
                parseFloat(row["settlement_currency_refunds_EUR"]) || 0,
              "#_of_refunds": parseInt(row["#_of_refunds"]) || 0,
              chargeback_won_amt_EUR:
                parseFloat(row["chargeback_won_amt_EUR"]) || 0,
              "#_of_chargebacks_won":
                parseInt(row["#_of_chargebacks_won"]) || 0,
              chargeback_issued_amt_EUR:
                parseFloat(row["chargeback_issued_amt_EUR"]) || 0,
            ...
            const multicurrencyFees =
              parseFloat(
                row["multicurrency_fees_EUR"] || row["multicurrency_fees_USD"],
              ) || 0;
            const perTransactionFees =
              parseFloat(
            ...
            const crossBorderFees =
              parseFloat(
                row["cross_border_fees_EUR"] || row["cross_border_fees_USD"],
              ) || 0;
            const payout =
              settlementSales +
              discount +
            ...
            newRows.push({
              ...
              disbursement_date: disbursementDate,
              settlement_currency_sales_EUR: row[
                "settlement_currency_sales_EUR"
              ]
                ? settlementSales
                : undefined,
              settlement_currency_sales_USD: row[
                "settlement_currency_sales_USD"
              ]
                ? settlementSales
                : undefined,
              discount_EUR: row["discount_EUR"] ? discount : undefined,
              discount_USD: row["discount_USD"] ? discount : undefined,
              multicurrency_fees_EUR: row["multicurrency_fees_EUR"]
                ? multicurrencyFees
                : undefined,
              multicurrency_fees_USD: row["multicurrency_fees_USD"]
                ? multicurrencyFees
                : undefined,
              per_transaction_fees_EUR: row["per_transaction_fees_EUR"]
                ? perTransactionFees
                : undefined,
              per_transaction_fees_USD: row["per_transaction_fees_USD"]
                ? perTransactionFees
                : undefined,
              cross_border_fees_EUR: row["cross_border_fees_EUR"]
                ? crossBorderFees
                : undefined,
              cross_border_fees_USD: row["cross_border_fees_USD"]
                ? crossBorderFees
                : undefined,
              other_fees_EUR:
                parseFloat(row["other_fees_EUR"] || row["other_fees_USD"]) || 0,
              chargebacks_lost_won_EUR:
                parseFloat(
              ...
              chargeback_fees_EUR:
                parseFloat(
                  row["chargeback_fees_EUR"] || row["chargeback_fees_USD"],
                ) || 0,
              payout: payout,
            });
            idCounter++;
            ...
            newRows.push({
              id:
                row["ID"] ||
                `${source.toUpperCase()}-${String(idCounter).padStart(3, "0")}`,
        ...
        const existingFileIndex = csvFiles.findIndex(
          (f) => f.source === source,
        );
        ...
        if (existingFileIndex >= 0) {
          ...
          updatedFiles[existingFileIndex].rows = [
            ...updatedFiles[existingFileIndex].rows,
            ...newRows,
          ];
  ...
  const saveEdit = async (fileIndex: number, rowId: string) => {
    const updatedFiles = [...csvFiles];
    const rowIndex = updatedFiles[fileIndex].rows.findIndex(
      (r) => r.id === rowId,
    );
    if (rowIndex !== -1) {
      updatedFiles[fileIndex].rows[rowIndex].category = editedCategory;
      updatedFiles[fileIndex].rows[rowIndex].classification =
        editedClassification;
      updatedFiles[fileIndex].rows[rowIndex].orderNumbers = editedOrderNumbers;
      updatedFiles[fileIndex].rows[rowIndex].depositAccount =
        editedDepositAccount;
      setCsvFiles(updatedFiles);
  ...
  const handleDeleteAllRows = async (fileIndex: number) => {
    if (
      !confirm(
        "Are you sure you want to delete ALL rows from this file? This action cannot be undone!",
      )
    )
      return;
  ...
  const handleDeleteAllReports = async () => {
    if (
      !confirm(
        "⚠️ WARNING: This will DELETE ALL DATA from ALL reports! This action CANNOT be undone! Are you absolutely sure?",
      )
    )
      return;
    if (
      !confirm(
        "⚠️ FINAL CONFIRMATION: All your financial data will be permanently deleted. Type YES to confirm.",
      )
    )
      return;
    ...
    try {
      ...
      if (result.success) {
        setCsvFiles([]);
        alert(
          "✅ All reports have been successfully deleted. You can now start fresh.",
        );
        // Recarregar dados (que estarão vazios)
        await loadData();
      } else {
        alert(
          "❌ Error deleting reports. Please check your Supabase configuration.",
        );
      }
    } catch (error) {
  ...
  const addClassification = () => {
    if (
      newClassification &&
      !classificationOptions.includes(newClassification)
    ) {
  ...
  const removeClassification = (classification: string) => {
    setClassificationOptions(
      classificationOptions.filter((c) => c !== classification),
    );
  };
  ...
  const addDepositAccount = () => {
    if (
      newDepositAccount &&
      !depositAccountOptions.includes(newDepositAccount)
    ) {
  ...
  const removeDepositAccount = (account: string) => {
    setDepositAccountOptions(
      depositAccountOptions.filter((a) => a !== account),
    );
  };
          ...
          <CardContent className="p-6">
            {files.length === 0 ? (
              <div className="text-center py-16 border-2 border-dashed border-[#e5e7eb] dark:border-[#2c3e5f] rounded-xl bg-gradient-to-br from-gray-50 to-white dark:from-slate-900 dark:to-slate-800">
                ...
                <p className="text-[#1a2b4a] dark:text-gray-300 mb-2 font-semibold text-lg">
              ...
              <div className="space-y-4">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  {files.length} file(s) uploaded. View detailed data in the
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-gray-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#1a2b4a] dark:text-white font-semibold">
            Loading data...
          </p>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-gray-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800">
      <Sidebar
        currentPage="home"
        paymentSourceDates={getPaymentSourceDates()}
      />
      ...
      <div className="md:pl-64">
              ...
              <div className="flex gap-3">
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      variant="outline"
                      className="gap-2 border-[#1a2b4a] text-[#1a2b4a] hover:bg-[#1a2b4a] hover:text-white dark:border-[#4fc3f7] dark:text-[#4fc3f7]"
                    >
                  ...
                  <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Manage classifications, payment methods, deposit
                        accounts, and custom columns
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <Tabs defaultValue="classifications" className="w-full">
                      <TabsList className="grid w-full grid-cols-4 bg-[#1a2b4a]/10">
                        ...
                        <TabsTrigger value="deposit-accounts">
                          Deposit Accounts
                        </TabsTrigger>
                        <TabsTrigger value="custom-columns">
                          Custom Columns
                        </TabsTrigger>
                      </TabsList>
                      ...
                      >
                        <div className="space-y-2">
                          <Label className="text-[#1a2b4a] dark:text-white font-semibold">
                          ...
                          <div className="flex gap-2">
                            <Input
                              ...
                              onKeyPress={(e) =>
                                e.key === "Enter" && addClassification()
                              }
                              className="border-[#1a2b4a]"
                            />
                            <Button
                              onClick={addClassification}
                              className="bg-[#1a2b4a] hover:bg-[#2c3e5f]"
                            >
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2 max-h-[300px] overflow-y-auto p-2 border-2 border-[#e5e7eb] dark:border-[#2c3e5f] rounded-lg">
                            {classificationOptions.map((classification) => (
                              <div
                                key={classification}
                                className="flex items-center justify-between p-3 bg-[#1a2b4a]/5 dark:bg-slate-800 rounded-lg border border-[#e5e7eb] dark:border-[#2c3e5f]"
                              >
                                <span className="text-sm font-medium text-[#1a2b4a] dark:text-white">
                                  {classification}
                                </span>
                                <Button
                                  size="sm"
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Input
                              ...
                              onChange={(e) =>
                                setNewDepositAccount(e.target.value)
                              }
                              onKeyPress={(e) =>
                                e.key === "Enter" && addDepositAccount()
                              }
                              className="border-[#1a2b4a]"
                            />
                            <Button
                      ...
                      <TabsContent value="custom-columns" className="space-y-4">
                        <div className="space-y-4">
                          ...
                          {newColumnType === "select" && (
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  setNewColumnOptions(e.target.value)
                                }
                                className="border-[#1a2b4a]"
                              />
                            </div>
                          ...
                          <Button
                            ...
                            className="w-full bg-[#1a2b4a] hover:bg-[#2c3e5f]"
                          >
        ...
        <div className="container mx-auto px-6 py-8">
          ...
          <Alert
            className="mb-8 border-2 border-[#4fc3f7] bg-[#4fc3f7]/10 shadow-lg"
            id="reconciliation"
          >
            ...
            <AlertDescription className="text-[#1a2b4a] dark:text-white font-medium">
              <strong>Reconciliation Status:</strong> Upload CSV files from each
              payment source to begin the reconciliation process. The system
              will automatically match transactions between sources.
            </AlertDescription>
          </Alert>
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              Bank account statements and transactions - view detailed data in
            ...
            <Alert className="border-2 border-blue-500 bg-blue-50 dark:bg-blue-900/20">
              ...
              <AlertDescription className="text-blue-800 dark:text-blue-200 font-medium">
                Bank statement data is managed in dedicated pages. Access{" "}
                <strong>Bankinter EUR</strong> and{" "}
                <strong>Bankinter USD</strong> from the sidebar menu under
                &quot;Bank Statements&quot;.
              </AlertDescription>
            </Alert>
          </div>
          ...
          <div className="mb-8">
            ...
            {/* Braintree Section */}
            <div className="mb-6">
              <h3 className="text-xl font-bold text-[#1a2b4a] dark:text-white mb-4 ml-4">
                Braintree
              </h3>
              {renderSourceSection(
                "braintree-eur",
          ...
          {/* Export Section */}
          <Card
            className="shadow-xl mt-8 bg-gradient-to-br from-[#4fc3f7]/10 to-[#00bcd4]/10 border-2 border-[#4fc3f7]"
            id="export"
          >
            <CardHeader>
              ...
              <CardDescription className="text-[#1a2b4a] dark:text-gray-300 font-medium">
                All data has been processed and is ready for export. Download
                the consolidated CSV file with all reconciled transactions.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <footer className="text-center text-sm text-gray-500 py-6">
          © {new Date().getFullYear()} DSD Finance Hub — Todos os direitos
          reservados.
        </footer>
      </div>
    </div>
