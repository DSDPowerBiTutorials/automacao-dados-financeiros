...
export default function BankinterEURPage() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        // Salvar no Supabase
        const totalAmount = updatedRows.reduce((sum, row) => sum + row.importe, 0)
  ...
  const downloadCSV = () => {
    const headers = [
      'ID', 'FECHA CONTABLE', 'FECHA VALOR', 'CLAVE', 'REFERENCIA', 
      'CATEGORÍA', 'DESCRIPCIÓN', 'REF. 12', 'REF. 16', 
      'DEBE', 'HABER', 'IMPORTE', 'SALDO', 'CONCILIADO'
    ]
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-[#e5e7eb] dark:border-[#2c3e5f] bg-gray-50 dark:bg-slate-800">
                      ...
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Fecha Contable</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Fecha Valor</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Clave</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Referencia</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Categoría</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Descripción</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">REF. 12</th>
                      <th className="text-left py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">REF. 16</th>
                      ...
                      <th className="text-right py-4 px-4 font-bold text-sm text-[#1a2b4a] dark:text-white">Importe</th>
                        ...
                        <td className="py-3 px-4 text-sm">
                          {editingRow === row.id ? (
                            <Input
                              ...
                              onChange={(e) => setEditedData({ ...editedData, fecha_contable: e.target.value })}
                              className="w-32"
                            />
                          ) : (
                            row.fecha_contable
                        ...
                        <td className="py-3 px-4 text-sm">
                          {editingRow === row.id ? (
                            <Input
                              ...
                              onChange={(e) => setEditedData({ ...editedData, fecha_valor: e.target.value })}
                              className="w-32"
                            />
                          ) : (
                            row.fecha_valor
