    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          if (rawRows.length <= 5) {
            alert('❌ File does not contain enough rows after header cleanup.')
            return
          }
          ...
          const fechaValorIndex = headerRow.findIndex(cell =>
            String(cell).toUpperCase().replace(/[ÃÁ]/g, 'A').includes('FECHA') &&
            String(cell).toUpperCase().includes('VALOR')
          )
          const descripcionIndex = headerRow.findIndex(cell =>
            String(cell).toUpperCase().replace(/[ÃÓÑ"]/g, 'O').includes('DESCRIPCI')
          )
          const haberIndex = headerRow.findIndex(cell => String(cell).toUpperCase() === 'HABER')
          const debeIndex = headerRow.findIndex(cell => String(cell).toUpperCase() === 'DEBE')
