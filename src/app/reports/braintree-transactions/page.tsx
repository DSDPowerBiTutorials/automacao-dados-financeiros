...
export default function BraintreeTransactionsPage() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        // Salvar no Supabase
        const totalAmount = updatedRows.reduce((sum, row) => sum + row.amount_authorized, 0)
  ...
  const downloadCSV = () => {
    const headers = [
      'ID', 'Payout', 'Transaction ID', 'Disbursement Date', 'Currency', 
      'Amount', 'Customer Name', 'Email', 'Order ID 1', 'Order ID 2', 
      'Order ID 3', 'Order ID 4', 'Bank Conciliation', 'Braintree EUR Conciliation'
    ]
        ...
        <div className="container mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card className="shadow-xl border-2 border-[#e5e7eb]">
              <div className="bg-gradient-to-br from-[#1a2b4a] to-[#2c3e5f] p-6">
                <p className="text-sm font-bold text-white/80 mb-2">Total Transactions</p>
          ...
          <Card className="shadow-xl">
            <CardHeader className="bg-gradient-to-r from-[#1a2b4a] to-[#2c3e5f] text-white">
              <CardTitle>All Braintree Transactions with Conciliation</CardTitle>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-[#e5e7eb] dark:border-[#2c3e5f] bg-gray-50 dark:bg-slate-800">
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">ID</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Transaction ID</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Date</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Currency</th>
                      <th className="text-right py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Amount</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Customer Name</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Email</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Order ID 1</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Order ID 2</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Order ID 3</th>
                      <th className="text-left py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Order ID 4</th>
                      <th className="text-center py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Bank Conciliation</th>
                      <th className="text-center py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Braintree EUR</th>
                      <th className="text-center py-4 px-3 font-bold text-xs text-[#1a2b4a] dark:text-white">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((row) => (
                      <tr key={row.id} className="border-b border-[#e5e7eb] dark:border-[#2c3e5f] hover:bg-gray-50 dark:hover:bg-slate-800/50">
                        <td className="py-3 px-3 text-xs font-bold">{row.id}</td>
                        <td className="py-3 px-3 text-xs">{row.transaction_id}</td>
                        <td className="py-3 px-3 text-xs">{row.disbursement_date}</td>
                        <td className="py-3 px-3 text-xs">
                          <Badge variant="outline">{row.currency_iso_code}</Badge>
                        </td>
                        <td className="py-3 px-3 text-xs text-right font-bold text-[#4fc3f7]">
                          €{row.amount_authorized.toFixed(2)}
                        </td>
                        <td className="py-3 px-3 text-xs">{row.customer_name}</td>
                        <td className="py-3 px-3 text-xs max-w-[150px] truncate">{row.customer_email}</td>
                        <td className="py-3 px-3 text-xs">{row.order_id_1 || '-'}</td>
                        <td className="py-3 px-3 text-xs">{row.order_id_2 || '-'}</td>
                        <td className="py-3 px-3 text-xs">{row.order_id_3 || '-'}</td>
                        <td className="py-3 px-3 text-xs">{row.order_id_4 || '-'}</td>
