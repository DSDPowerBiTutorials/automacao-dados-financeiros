...
export default function BraintreeAUDPage() {
  ...
  const [editedData, setEditedData] = useState<Partial<BraintreeAUDRow>>({});
  ...
  const loadLastSyncDate = async () => {
    try {
      const { data, error } = await supabase
        .from("csv_rows")
        .select("created_at")
        .or("source.eq.braintree-api-revenue,source.eq.braintree-aud")
  ...
  const reconcileBankStatements = async (
    ...
    try {
      ...
      // Reconciliar cada linha do Braintree AUD
  ...
  const loadData = async (options: { runReconcile?: boolean } = {}) => {
    ...
    try {
      if (!supabase) {
        console.error("[Braintree AUD] Supabase not configured!");
        setRows([]);
        return;
      }

      console.log("[Braintree AUD] Fetching data from Supabase...");
      ...
      if (error) {
        console.error("[Braintree AUD] Error loading data:", error);
        setRows([]);
        return;
      }

      if (!rowsData || rowsData.length === 0) {
        console.log("[Braintree AUD] No data found");
        setRows([]);
        return;
      }

      console.log(`[Braintree AUD] Found ${rowsData.length} rows`);
      ...
      console.log("[Braintree AUD] Data loaded successfully");

      // Carregar última data de sync (sem bloquear)
      loadLastSyncDate().catch(err => console.error("[Braintree AUD] Error loading sync date:", err));
    } catch (error) {
      console.error("[Braintree AUD] Unexpected error:", error);
      setRows([]);
    } finally {
      console.log("[Braintree AUD] Setting isLoading to false");
      setIsLoading(false);
    }
  };
  ...
  const startEditing = (row: BraintreeAUDRow) => {
  ...
  const downloadCSV = () => {
    try {
      ...
      a.href = url;
      a.download = `braintree-aud-${new Date().toISOString().split("T")[0]}.csv`;
  ...
  return (
    <div className="min-h-full">
      ...
      >
        <PageHeader title="Braintree AUD - Payment Source" subtitle={`${rows.length} records (${processedRows.length} filtered) - Page ${adjustedCurrentPage} of ${Math.max(1, totalPages)}`}>
          ...
          <Card className="shadow-xl">
            <CardHeader className="bg-gradient-to-r from-[#1a2b4a] to-[#2c3e5f] text-white">
              ...
              <CardDescription className="text-gray-900 dark:text-white/80">
                Manage Braintree AUD transactions with filtering and sorting
              </CardDescription>
            </CardHeader>
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="digitalsmiledesignAUD">digitalsmiledesignAUD</SelectItem>
