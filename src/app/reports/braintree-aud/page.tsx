...
export default function BraintreeAUDPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Filtrar apenas transações em AUD
      const audRows = rowsData.filter(
        (row) => row.custom_data?.currency === "AUD"
      );

      const mappedRows: BraintreeAUDRow[] = audRows.map((row) => ({
        id: row.id,
        date: row.date,
  ...
  const handleFileUpload = async (
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        console.log("=== BRAINTREE AUD CSV PROCESSING ===");
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          newRows.push({
            ...
            description: `Braintree AUD Disbursement - ${disbursementDate}`,
            amount: payout,
            conciliado: false,
            destinationAccount: null,
            reconciliationType: null,
  ...
  const handleDeleteAll = async () => {
    if (
      !confirm(
        "⚠️ WARNING: This will DELETE ALL rows from Braintree AUD! Are you sure?",
      )
    )
      return;
