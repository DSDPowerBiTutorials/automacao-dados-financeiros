...
export default function BraintreeGBPPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Filtrar apenas transações em GBP
      const gbpRows = rowsData.filter(
        (row) => row.custom_data?.currency === "GBP"
      );

      const mappedRows: BraintreeGBPRow[] = gbpRows.map((row) => ({
        id: row.id,
        date: row.date,
  ...
  const handleFileUpload = async (
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        console.log("=== BRAINTREE GBP CSV PROCESSING ===");
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          newRows.push({
            ...
            description: `Braintree GBP Disbursement - ${disbursementDate}`,
            amount: payout,
            conciliado: false,
            destinationAccount: null,
            reconciliationType: null,
  ...
  const handleDeleteAll = async () => {
    if (
      !confirm(
        "⚠️ WARNING: This will DELETE ALL rows from Braintree GBP! Are you sure?",
      )
    )
      return;
