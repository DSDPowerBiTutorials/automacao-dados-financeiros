...
export default function BraintreeUSDPage() {
  ...
  const loadData = async (options: { runReconcile?: boolean } = {}) => {
    ...
    try {
      if (!supabase) {
        console.error("[Braintree USD] Supabase not configured!");
        setRows([]);
        return;
      }

      console.log("[Braintree USD] Fetching data from Supabase...");
      ...
      if (error) {
        console.error("[Braintree USD] Error loading data:", error);
        setRows([]);
        return;
      }

      if (!rowsData || rowsData.length === 0) {
        console.log("[Braintree USD] No data found");
        setRows([]);
        return;
      }

      console.log(`[Braintree USD] Found ${rowsData.length} rows`);

      const mappedRows: BraintreeUSDRow[] = rowsData
        .filter((row) => {
      ...
      console.log("[Braintree USD] Data loaded successfully");

      // Carregar última data de sync (sem bloquear)
      loadLastSyncDate().catch(err => console.error("[Braintree USD] Error loading sync date:", err));
    } catch (error) {
      console.error("[Braintree USD] Unexpected error:", error);
      setRows([]);
    } finally {
      console.log("[Braintree USD] Setting isLoading to false");
      setIsLoading(false);
    }
  };
  ...
  const processedRows = useMemo(() => {
    ...
    let processedRows = filteredRows.map((row: BraintreeUSDRow, index: number, array: BraintreeUSDRow[]) => {
      const disbursementId = row.disbursement_id || 'ungrouped';
    ...
    // Sorting
    processedRows.sort((a: BraintreeUSDRow, b: BraintreeUSDRow) => {
      let comparison = 0;
  ...
  return (
    <div className="min-h-full">
      ...
      >
        <PageHeader title="Braintree USD - Payment Source" subtitle={`${rows.length} records (${processedRows.length} filtered) - Page ${adjustedCurrentPage} of ${Math.max(1, totalPages)}`}>
                      ...
                      {(() => {
                        ...
                        if (!group) {
                          return (
                            <p className="text-sm text-gray-600">
                              {processedRows.length} transactions • Total: ${processedRows.reduce((sum: number, r: BraintreeUSDRow) => sum + r.amount, 0).toFixed(2)}
                            </p>
                          );
                        }
                        ...
                        return (
                          <div className="space-y-3">
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-black rounded-lg">
                ...
                <table className="w-full min-w-max table-auto">
                  ...
                  <tbody>
                    {paginatedRows.length === 0 ? (
                      ...
                      paginatedRows.map((row: BraintreeUSDRow) => {
                                  ...
                                  <div className="flex items-center justify-center gap-2">
                                    {row.reconciliationType === "automatic" ? (
                                      <div className="relative group">
                                        ...
                                        <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-50 dark:bg-black text-gray-900 dark:text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 max-w-xs">
