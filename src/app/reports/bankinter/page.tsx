...
export default function BankinterPage() {
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          console.log('💰 HABER conversion:', { original: haberStr, converted: haberValue })
          ...
          newRows.push({
            ...
            amount: haberValue,
            fecha_contable: row['FECHA CONTABLE'] || row['FECHA_CONTABLE'] || '',
  ...
  const handleDeleteAll = async () => {
    if (!confirm('⚠️ WARNING: This will DELETE ALL rows from Bankinter! Are you sure?')) return
  ...
  const downloadCSV = () => {
    const headers = [
      'ID', 'Date', 'Description', 'Amount', 'Fecha Contable', 'Fecha Valor', 
      'Clave', 'Referencia', 'Categoria', 'Debe', 'Haber', 'Importe', 'Saldo'
    ]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-gray-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800">
      <Sidebar currentPage="bankinter" paymentSourceDates={{}} />
