...
export default function PaypalPage() {
  ...
  const handleFileUpload = async (
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        const netoIndex = headers.findIndex((h) =>
          h.toLowerCase().includes("neto"),
        );
        ...
        if (fechaIndex === -1 || netoIndex === -1) {
          alert(
            "❌ Required columns not found! Make sure the file has: Fecha, Neto",
          );
  ...
  const handleDeleteAll = async () => {
    if (
      !confirm(
        "⚠️ WARNING: This will DELETE ALL rows from Paypal! Are you sure?",
      )
    )
      return;
  ...
  return (
    <div className="min-h-full">
      ...
      >
        <PageHeader title="PayPal - Payment Source" subtitle={`${rows.length} records`}>
          ...
          <Card className="shadow-xl">
            <CardHeader className="bg-gradient-to-r from-[#1a2b4a] to-[#2c3e5f] text-white">
              ...
              <CardDescription className="text-gray-900 dark:text-white/80">
                Upload CSV files - Columns: Fecha → Date | Neto → Amount (only
                positive values)
              </CardDescription>
            </CardHeader>
