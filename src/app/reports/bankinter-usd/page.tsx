...
// 👉 helper para deixar tudo como dd/MM/yyyy
function formatDate(dateStr: string | null | undefined): string {
  ...
  // formato dd/mm/aaaa ou dd-mm-aaaa → normaliza para dd/mm/aaaa
  ...
  // qualquer outra coisa, devolve como veio
  return dateStr
}
...
export default function BankinterUSDPage() {
    ...
    try {
      ...
      if (error) {
        ...
        const mappedRows: BankinterUSDRow[] = rowsData.map((row) => ({
          id: row.id,
          // 👇 já normaliza para dd/MM/yyyy aqui
    ...
    if (files && files.length > 0) {
      ...
      reader.onload = async (e) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const fechaValorRaw = (values[fechaValorIndex] || "").trim()
          ...
          const uniqueId = `BANKINTER-USD-${Date.now()}-${Math.random()
            .toString(36)
            .substr(2, 9)}`

          newRows.push({
            id: uniqueId,
            // 👇 normaliza a data lida do CSV
            date: formatDate(fechaValorRaw),
  ...
  const handleDeleteAll = async () => {
    if (!confirm("⚠️ WARNING: This will DELETE ALL rows from Bankinter USD! Are you sure?"))
      return
    ...
    try {
      ...
      const csvContent = [
        ...
        ...rows.map((row) =>
          [
            row.id,
            row.date, // já vem como dd/MM/yyyy
          ...
          <Card className="shadow-xl">
            <CardHeader className="bg-gradient-to-r from-[#1a2b4a] to-[#2c3e5f] text-white">
              ...
              <CardDescription className="text-white/80">
                Upload CSV files - Columns: FECHA VALOR → Date | DESCRIPCIÓN → Description | HABER →
                Amount
              </CardDescription>
            </CardHeader>
