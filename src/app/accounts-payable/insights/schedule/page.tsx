...
type DaySubGroup = {
    dayKey: string; // YYYY-MM-DD or "no-date"
    dayLabel: string;
    invoices: Invoice[];
    totalEUR: number;
    totalUSD: number;
...
export default function PaymentSchedulePage() {
    ...
    function getDaySubGroups(group: ScheduleGroup): DaySubGroup[] {
        if (group.date === "unscheduled") {
            return [{ dayKey: "no-date", dayLabel: "No date", invoices: group.invoices, totalEUR: group.totalEUR, totalUSD: group.totalUSD }];
        }
        const dayMap = new Map<string, DaySubGroup>();
        group.invoices.forEach((inv) => {
            const dayKey = inv.schedule_date || "no-date";
            if (!dayMap.has(dayKey)) {
                dayMap.set(dayKey, {
                    dayKey,
                    dayLabel: dayKey === "no-date" ? "No date" : formatDayHeader(dayKey),
                    invoices: [],
                    totalEUR: 0,
                    totalUSD: 0,
                });
            }
            const sub = dayMap.get(dayKey)!;
            sub.invoices.push(inv);
            if (inv.currency === "EUR") sub.totalEUR += inv.invoice_amount;
            else if (inv.currency === "USD") sub.totalUSD += inv.invoice_amount;
        });
    ...
    async function updateScheduleDate(invoiceId: number, date: string | null) {
        ...
        try {
            ...
            if (date) {
                setExpandedGroups((prev) => new Set([...prev, getMonthKey(date)]));
    ...
    async function updateInvoiceField(invoiceId: number, field: string, value: any) {
        ...
        try {
            ...
            // Expand group + day if schedule_date changed
            if (field === "schedule_date" && value) {
                setExpandedGroups((prev) => new Set([...prev, getMonthKey(value)]));
                setExpandedDays((prev) => new Set([...prev, value]));
            }
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white flex">
            ...
            {detailPanelOpen && selectedInvoice && (
                <div className="fixed right-0 top-[64px] h-[calc(100vh-64px)] w-[500px] bg-white dark:bg-black border-l border-gray-200 dark:border-gray-700 flex flex-col z-[1040] shadow-2xl">
                    {/* Panel Header */}
