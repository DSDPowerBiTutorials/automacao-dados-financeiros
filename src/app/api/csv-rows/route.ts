...
function getSupabaseAdmin() {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    ...
    console.error('URL:', supabaseUrl ? '✅' : '❌ NEXT_PUBLIC_SUPABASE_URL not found')
    console.error('Service Key:', supabaseServiceKey ? '✅' : '❌ SUPABASE_SERVICE_ROLE_KEY not found')
    return null
  }
    ...
    if (!supabaseAdmin) {
      return NextResponse.json(
        { 
          ...
          details: 'Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Please restart the development server.'
        },
        { status: 500 }
      )
    }
