...
function parseDate(value: any): string | null {
  ...
  if (typeof value === 'number') {
    ...
    return new Date(Date.UTC(dateObj.y, dateObj.m - 1, dateObj.d)).toISOString().split('T')[0]
  }
...
async function parseUploadedFile(file: File): Promise<any[][]> {
  ...
  return XLSX.utils.sheet_to_json(parsedSheet, { header: 1, defval: '' }) as any[][]
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file || !(file instanceof File)) {
      return NextResponse.json({ success: false, error: 'Arquivo não enviado ou inválido.' }, { status: 400 })
    }

    const data = await parseUploadedFile(file)

    let filteredData = data.slice(HEADER_OFFSET).filter(row => row.some(cell => String(cell ?? '').trim() !== ''))

    const endIndex = filteredData.findIndex(row => String(row[0] ?? '').toUpperCase().includes('INFORMACIÓN DE INTERÉS'))
    if (endIndex !== -1) {
    ...
    const headers = (filteredData[0] as string[]).map(header => header.trim().toUpperCase())
    ...
    if (fechaValorIndex === -1 || descripcionIndex === -1) {
      throw new Error('❌ Colunas obrigatórias não encontradas (FECHA VALOR / DESCRIPCIÓN).')
    }
    ...
    console.log(`✅ Upload concluído: ${FILE_NAME} (${parsedRows.length} registros)`) // eslint-disable-line no-console

    return NextResponse.json({ success: true, inserted: parsedRows.length })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erro desconhecido ao processar upload.'
