...
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === "fetch-by-ids") {
            const { ids, select = "id, customer_name, order_id, invoice_number, amount, financial_account_code, products" } = body;
        ...
        if (action === "fetch-by-order-id") {
            const { order_id, select = "id, customer_name, order_id, invoice_number, amount, financial_account_code, products" } = body;
            const { data, error } = await supabaseAdmin
                .from("ar_invoices")
                .select(select)
