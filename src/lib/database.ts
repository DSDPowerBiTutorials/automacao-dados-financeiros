...
export async function saveCSVFile(file: CSVFile) {
  try {
    ...
    if (!file.name || !file.source || !file.rows || file.rows.length === 0) {
      throw new Error(
        "Dados inválidos: arquivo deve ter nome, source e pelo menos uma linha",
      );
    }
...
export async function loadAllCSVFiles(): Promise<CSVFile[]> {
  ...
  try {
    ...
    const csvFiles: CSVFile[] = files.map((file) => {
      const fileRows = rows
        .filter(
          (row) => row.file_name === file.name && row.source === file.source,
        )
        .map((row) => ({
          id: row.id,
          date: row.date,
...
export async function deleteCSVFile(fileName: string, source: string) {
  try {
    const response = await fetch(
      `/api/csv/delete?fileName=${encodeURIComponent(fileName)}&source=${source}`,
      {
        method: "DELETE",
      },
    );
