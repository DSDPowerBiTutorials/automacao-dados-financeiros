...
export function downloadFinalCSV(csvFiles: CSVFile[]) {
  ...
  let csvContent =
    "ID,Date,Description,Amount,Category,Classification,Source,DepositAccount,PaymentMethod,OrderNumbers,Reconciled,MatchedWith\n";
  ...
  try {
    ...
    link.setAttribute(
      "download",
      `reconciled_data_${new Date().toISOString().split("T")[0]}.csv`,
    );
...
export function downloadIndividualCSV(file: CSVFile) {
  ...
  const isBankStatement =
    file.source === "bankinter-eur" ||
    file.source === "bankinter-usd" ||
    file.source === "sabadell";
  ...
  try {
    ...
    if (file.source === "braintree-amex") {
      csvContent =
        "ID,Date,Description,Amount,Category,Classification,Source,DepositAccount,OrderNumbers,settlement_date,settlement_number,total_charges,credits,submission_amount,discount_amount,fees_and_incentives,chargebacks,adjustments,held_funds,settlement_amount,settlement_currency_code,amount_paid_to_bank\n";
    ...
    // Para Stripe, incluir todas as colunas específicas
      ...
      csvContent =
        "ID,Date,Description,Amount,Category,Classification,Source,DepositAccount,OrderNumbers,automatic_payout_effective_at,net\n";
    ...
    // Para GoCardless, incluir todas as colunas específicas
      ...
      csvContent =
        "ID,Date,Description,Amount,Category,Classification,Source,DepositAccount,OrderNumbers,payouts.arrival_date,net_amount\n";
    ...
    // Para Braintree EUR, incluir todas as colunas específicas
      ...
      csvContent =
        "ID,Date,Description,Payout,Category,Classification,Source,DepositAccount,OrderNumbers,disbursement_date,settlement_currency_sales_EUR,discount_EUR,multicurrency_fees_EUR,per_transaction_fees_EUR,cross_border_fees_EUR,other_fees_EUR,chargebacks_lost_won_EUR,settlement_currency_refunds_EUR,#_of_refunds,chargeback_won_amt_EUR,#_of_chargebacks_won,chargeback_issued_amt_EUR,#_of_chargebacks_issued,chargeback_fees_EUR\n";
    ...
    // Para Braintree USD, incluir todas as colunas específicas (suporta ambos os formatos)
      ...
      file.rows.forEach((row) => {
        ...
        const multicurrencyFees =
          row.multicurrency_fees_EUR || row.multicurrency_fees_USD || 0;
        const perTransactionFees =
          row.per_transaction_fees_EUR || row.per_transaction_fees_USD || 0;
        const crossBorderFees =
          row.cross_border_fees_EUR || row.cross_border_fees_USD || 0;
    ...
    link.setAttribute(
      "download",
      `${file.name.replace(".csv", "")}_reconciled.csv`,
    );
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
